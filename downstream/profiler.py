...
# Adapted from:
# https://github.com/k2-fsa/icefall/blob/42de4591107868816d240c991b1e8acdfdfd090b/icefall/profiler.py
...
def profile(
    ...
    _LOGGER.warning(f"fp16: {fp16}")

    # Default PyTorch value is usually OK
    if num_threads is not None:
        torch.set_num_threads(num_threads)
    _LOGGER.warning(f"Number of threads: {torch.get_num_threads()}")
    ...
    for _ in range(num_warmups):
        for input_shape in input_shapes:
            inputs = input_fn(input_shape, device=device)
    ...
    for input_shape in input_shapes:
        _LOGGER.info(f"Benchmarking input of shape {input_shape}")
        ...
        memory_results.append(total_peak_memory / num_runs)

        # This is deterministic, no need to average
        try:
            with torch.inference_mode():
                with torch.autocast(device_type=device_type, enabled=fp16):
                    flops, _ = get_model_profile(
                        model=model,
                        args=[inputs],
                        as_string=False,
                    )
                    macs = 0.5 * flops
                    macs_results.append(macs / 1e9)

        except Exception as e:
            _LOGGER.warning(e)

        # Double check with ptflops if available
        try:
            import ptflops
    ...
    for x in model.state_dict().values():
        if x.requires_grad:
            trainable_params += x.numel()
        total_params += x.numel()

    results = {
        "time (s)": time_results,
        "memory (GB)": memory_results,
        "macs (GMACS)": macs_results,
    ...
    for input_shape in input_shapes:
        ...
        total_time = timer.timeit(num_runs).mean
        time_results2.append(total_time)

    results["time (s) (PyTorch timer)"] = time_results2
    results["macs (GMACS) (ptflops)"] = macs_results2
...
def _sum_flops_compute(input, dim=None, keepdim=False):
    return input.numel()
