...
"""HILCodec (see https://arxiv.org/abs/2405.04752)."""
...
class HILCodec(Codec):
    MODEL_NAMES = ["hilcodec_speech", "hilcodec_music"]
    ...
    def __init__(
        ...
        try:
            ...
            from hilcodec.models import HILCodec as HILCodecOrig

            sys.path = sys_path
        except ImportError:
            raise ImportError(
                "`pip install git+https://github.com/lucadellalib/hilcodec.git` to use this module"
            )

        super().__init__(sample_rate, 0, mode)
        self.num_codebooks = num_codebooks
        self.vocab_size = 1024

        self.model = HILCodecOrig.from_pretrained(model_name)
        self.orig_sample_rate = self.model.sample_rate
        if mode == "encode":
    ...
    def _sig_to_toks(self, sig, length):
        ...
        qfeats, _, _, toks = self.model.quantizer(feats, n=self.num_codebooks)
...
if __name__ == "__main__":
    ...
    codec = HILCodec(sample_rate, num_codebooks=num_codebooks).eval()
    with torch.no_grad():
        rec_sig = codec(sig)
